...
\\This causes a small jump at \(r = 3\sigma\) however, since the potential at this distance is not exactly 0. The remedy here is to simply shift the whole potential by the term \(U(r=3.0)\), leading to a smooth curve up until \(r=3\sigma\).

\begin{enumerate}[label=\roman*., resume]
    \item Extend your implementation such that atoms more than \(3\sigma\) apart do not interact.
    \item Plot again \(U(r)\) and \(F(r)\). Does the shift of the potential described above impact the force calculations?
...
\begin{enumerate}[label=\roman*.]
 ...
 \item Start with a 4-atom system. Place the atoms so that $\vec{r_1} = 2\vec{i}$, $\vec{r_2} = -2\vec{i}$, $\vec{r_3} = 2\vec{j}$ and $\vec{r_4} = -2\vec{j}$. Visualize with Ovito, and explain the results. Did the system behave as you expected?
 \item Calculate the potential and kinetic energy of the same system, and comment on the conservation of energy.
 \item Do the same again, but now perturb \textit{one} of the atoms, giving them a small positional component (say 0.1) in one of the directions where it originally was 0. Visualize the results,
