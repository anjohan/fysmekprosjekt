...
\begin{equation}
    A(t) = \frac{\ev{\vec{v}(t)\cdot\vec{v}(0)}}{\ev{\vec{v}(0)\cdot\vec{v}(0)}}
         = \frac{1}{N}  \frac{\sum_i \vec{v}_i(t)\cdot\vec{v}_i(0)}{\sum_i \vec{v}_i(0)\cdot\vec{v}_i(0)},\label{eq:vacf}
\end{equation}
...
A couple of optional exercises might be added here. First, the pressure is a natural quantity to measure, but was not done by \cite{Rahman_1964}. This calculation also requires the definition of the virial, which might be confusing. Second, the velocity distribution at equilibrium could be found.
